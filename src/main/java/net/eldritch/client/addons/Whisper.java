...
public class Whisper {
	...
	public static void WhisperInit() {
		...
		FabricKeyBinding toggleHotkey = FabricKeyBinding.Builder
				.create(new Identifier("eldritchclient", "toggle.whisper"), InputUtil.Type.KEYSYM, -1, "eldritchclient")
				.build();
	...
	/**
	 * Returns an index if the strings match, where the regex has underscores rather
	 * than spaces (and no spaces) and {} in the regex counts as any string without
	 * spaces Returns -1 otherwise The index is the length of the total matching
	 * string, as {} may make this not the length of regex
	 */
	public static int specialStartsWith(String regex, String string) {
		int ri = 0;
		int si = 0;
		while (true) {
			if (ri == regex.length() && (si == string.length() || string.charAt(si) == ' '))
				return si;
			if (si == string.length())
				return -1;
			...
			if (regex.charAt(ri) == string.charAt(si)) {
				ri++;
				si++;
				continue;
			}
			if (string.charAt(si) == ' ' && regex.charAt(ri) == '_') {
				ri++;
				si++;
				continue;
			}
	...
	public static boolean workOnMessage(String string) {
		...
		for (String ignore : ignoreList)
			if (ignore.length() > 0 && string.startsWith(ignore))
				return false;
		return true;
	}
